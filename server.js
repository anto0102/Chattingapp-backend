...
const geoip = require('geoip-lite'); // MODIFICA: Importiamo la libreria GeoIP

const app = express();
app.use(cors());
const server = http.createServer(app);
// ... (il resto della configurazione iniziale rimane invariato)
const io = new Server(server, { cors: { origin: "*", methods: ["GET", "POST"] } });
const BANNED_IPS_FILE = path.join(__dirname, "banned-ips.json"); const REPORTS_FILE = path.join(__dirname, "reports.json");
...
app.use("/admin", basicAuth({ users: { admin: "changeme" }, challenge: true })); app.use("/adminreport", basicAuth({ users: { admin: "changeme" }, challenge: true }));
app.get("/admin", (req, res) => res.sendFile(path.join(__dirname, "admin.html"))); app.get("/adminreport", (req, res) => res.sendFile(path.join(__dirname, "adminreport.html"))); app.get("/reports.json", (req, res) => res.json(reports));
...
// ... (il resto del codice fino a io.on("connection", ...) rimane invariato)
let connectedUsers = {}; let waitingUser = null; let activeChats = {};
function emitOnlineCount() { const currentOnlineUsers = Object.values(connectedUsers).filter(u => !u.isAdmin).length; io.emit('online_count', currentOnlineUsers); console.log(`Aggiornato conteggio online: ${currentOnlineUsers}`); }
io.use((socket, next) => { const ip = getClientIP(socket); if (bannedIPs.has(ip)) return next(new Error("BANNED")); next(); });
...
io.on("connection", (socket) => {
  ...
  if (!isAdmin) {
    socket.on("start_chat", () => {
      if (waitingUser && waitingUser.connected) {
        ...
        // Usiamo il codice del paese (es. "IT") o "Sconosciuto" se non trovato
    ...
    // ... (il resto del codice del server rimane invariato)
    socket.on("message", (msgText) => { if (socket.partner && socket.partner.connected) { const messageObject = { id: uuidv4(), text: msgText, senderId: socket.id, timestamp: new Date(), reactions: {} }; if (socket.room && activeChats[socket.room]) { activeChats[socket.room].messages.push(messageObject); } io.to(socket.id).to(socket.partner.id).emit("new_message", messageObject); console.log(`Messaggio [${messageObject.id}] da ${socket.id}`); } else { socket.emit("partner_disconnected"); socket.partner = null; } });
    socket.on('add_reaction', ({ messageId, emoji }) => { const room = socket.room; if (!room || !activeChats[room] || !socket.partner) return; const message = activeChats[room].messages.find(m => m.id === messageId); if (!message) return; for (const existingEmoji in message.reactions) { if (existingEmoji !== emoji && message.reactions[existingEmoji].has(socket.id)) { message.reactions[existingEmoji].delete(socket.id); } } if (!message.reactions[emoji]) { message.reactions[emoji] = new Set(); } if (message.reactions[emoji].has(socket.id)) { message.reactions[emoji].delete(socket.id); } else { message.reactions[emoji].add(socket.id); } const reactionsForClient = {}; for (const key in message.reactions) { const count = message.reactions[key].size; if (count > 0) { reactionsForClient[key] = count; } } io.to(socket.id).to(socket.partner.id).emit('update_reactions', { messageId, reactions: reactionsForClient }); });
  ...
  socket.on("disconnect", () => { const user = connectedUsers[socket.id]; if (user && user.socket.room) delete activeChats[user.socket.room]; if (user && user.socket.partner) { const partnerSocket = user.socket.partner; if (partnerSocket.connected) { partnerSocket.emit("partner_disconnected"); partnerSocket.partner = null; partnerSocket.room = null; } } delete connectedUsers[socket.id]; if (waitingUser === socket) waitingUser = null; updateAdminUI(); emitOnlineCount(); });
});
